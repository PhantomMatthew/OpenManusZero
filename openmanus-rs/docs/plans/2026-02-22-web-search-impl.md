...
Run: `cd openmanus-rs && cargo check`
Expected: Compiles with warnings about unused code

**Step 7: Commit**

```bash
cd openmanus-rs && git add Cargo.toml src/tool/web_search/ src/tool/mod.rs && git commit -m "feat(tool): add WebSearch module structure"
```
...
Run: `cd openmanus-rs && cargo test duckduckgo -- --nocapture`
Expected: Tests pass (may be slow due to network)

**Step 3: Commit**

```bash
cd openmanus-rs && git add src/tool/web_search/duckduckgo.rs && git commit -m "feat(tool): implement DuckDuckGo search engine"
```
...
Run: `cd openmanus-rs && cargo test fetcher -- --nocapture`
Expected: Tests pass

**Step 3: Commit**

```bash
cd openmanus-rs && git add src/tool/web_search/fetcher.rs && git commit -m "feat(tool): implement content fetcher"
```
...
Run: `cd openmanus-rs && cargo test web_search -- --nocapture`
Expected: Tests pass

**Step 3: Commit**

```bash
cd openmanus-rs && git add src/tool/web_search/tool.rs && git commit -m "feat(tool): implement WebSearchTool with fallback support"
```
...
```bash
cd openmanus-rs && git add src/tool/collection.rs pending_tasks.md && git commit -m "feat(tool): add WebSearchTool to default collection"
```
