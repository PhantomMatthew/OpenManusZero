...
```bash
cd openmanus-rs && git add src/agent/swe.rs src/agent/mod.rs && git commit -m "feat(agent): add SweAgent skeleton"
```
...
Run: `cd openmanus-rs && cargo test swe -- --nocapture`
Expected: All 5 tests pass

**Step 3: Commit**

```bash
cd openmanus-rs && git add src/agent/swe.rs && git commit -m "feat(agent): add SweAgent unit tests"
```
...
```bash
cd openmanus-rs && git add pending_tasks.md && git commit -m "docs: mark SWEAgent as complete"
```
